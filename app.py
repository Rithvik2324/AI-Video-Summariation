...
def extract_audio_from_youtube(youtube_url, output_audio_filename="output_audio.wav"):
    ...
    try:
        ...
        audio_file_clip.write_audiofile(audio_filename, logger=None)
...
def summarize_with_ollama(text, model_name="llama2"):
    ...
    try:
        ...
        return response['message']['con             tent'].strip()
    except ImportError:
        return "Ollama library not installed. Please install it using 'pip install ollama'"
    except Exception as e:
        return f"Ollama summarization error: {e}"

ASSEMBLYAI_API_KEY = "a42dc409a1db491a80ded63ada999301"

@app.route('/')
def index():
    return render_template('youtube_ui.html')
...
def process_youtube_link():
    ...
    try:
        ...
        if isinstance(audio_file_path, str) and audio_file_path.startswith(("yt-dlp Error", "An error occurred")):
            return jsonify({'error': f"Audio Extraction Error: {audio_file_path}"})

        transcription_result = transcribe_assemblyai(audio_file_path, ASSEMBLYAI_API_KEY)
        if isinstance(transcription_result, str) and transcription_result.startswith(("Request error", "JSON decode error", "Error", "Transcription failed")):
            return jsonify({'error': f"Transcription Error: {transcription_result}"})

        summary_result = summarize_with_ollama(transcription_result, model_name)
        if isinstance(summary_result, str) and summary_result.startswith("Ollama summarization error"):
            return jsonify({'error': f"Summarization Error: {summary_result}"})

        return jsonify({'transcription': transcription_result, 'summary': summary_result})
